...
# Keyword collocates and concordances
...
10 words of the keyword than in other places in the submissions.
...
single_word_group_concordance <- function(i) {
  feature <- keyword_sources$feature[i]
  group <- keyword_sources$keyword_from[i]
  ...
  return(concordances %>% select(docname, keyword_from, pattern, pre, keyword, post, merged))
}

concordances <- bind_rows(lapply(seq_along(keyword_sources$feature), single_word_group_concordance))
