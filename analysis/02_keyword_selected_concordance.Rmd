...
extract_collocations <- function(i) {
  ...
  # Note we're also excluding low document frequency words for collocates.
  # The threshold is a bit lower to allow a bit more diversity in the
  # collocating words.
  # We're also determining collocations only for the group that generated
  # the keyword so we can understand it in context for that group.
  dfm_inside <- dfm_trim(dfm(tokens_inside), min_docfreq=3)
  dfm_outside <- dfm_trim(dfm(tokens_outside), min_docfreq=3)
...
plot_groupwise_rel_freqs <- function (args) {
  ...
  keywords = args[[2]]
  subset <- relative_frequencies %>% filter(feature %in% keywords)
  ...
  small_multi <-
    ...
    filename <- paste(c("../results/group_comparison", highlight_group, paste(keywords, collapse="_"), ".pdf"), collapse="_")
...
groups_to_visualise <- list(
  list('consumptive', c("acres", "barley", "cattle", "cereals", "dairy", "growers", "pasture", "planted")),
...
single_word_group_concordance <- function(feature, groups) {
  ...
  for (group in groups) {
    ...
    if (nrow(idx) > 0) {
      ...
      concordances$keyword_from <- group
      concordances <- concordances %>% 
        select(docname, keyword_from, pattern, pre, keyword, post, merged)
...
concordances <- bind_rows(lapply(keyword_sources$feature, single_word_group_concordance, groups=unique(keyword_sources$keyword_from)))
