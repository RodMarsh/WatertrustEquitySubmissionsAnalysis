...
selected_subset <- filtered_rel_freq %>% 
  filter(feature %in% include_keywords$feature, label %in% labels) %>%
...
# For each of the keywords remaining after filtering, find the cooccurring words
...
for (word in illustrative_words) {
  # Make the binary weighted token windows for the selected words
  token_windows <- tokens_select(
    submission_tokens, 
    pattern=word, 
    valuetype='fixed', 
    # Note that window = 10 is after removing stopwords and punctuation, so
  ...
  for (group in chosen_groups) {
    ...
    group_score$chosen_word <- word
    group_score$group <- group
    keyword_group_ranks <- append(keyword_group_ranks, list(group_score))
  }
}

output_ranked_collocates <- bind_rows(keyword_group_ranks)

write_xlsx(output_ranked_collocates, "../results/selected_group_collocation_scores.xlsx")
