...
include_keywords <- data.frame(feature=keywords, keyword_from=keyword_labels)
...
selected_subset <- filtered_rel_freq %>% 
  ...
  mutate(keyword_fill = "bg", highlight=ifelse(keyword_from==label, 'highlight', 'bgalpha'))

selected_subset$feature <- factor(selected_subset$feature, keywords)
