...
This notebook represents the initial set of explorations used to start working
out different features and approaches that might be relevant for the
different inquiry datasets.
...
read_pdfs_in_dir <- function(dir) {
  list.files(path = dir, pattern = "(?i)\\.pdf$", full.names = TRUE) %>% 
    map_df(~{
      tryCatch({
        text <- pdf_text(.x)
        clean_text <- gsub("[^[:print:]]", "", text)
...
# identify the equity submissions and incorporate into the topic models

pc_topic_models <- map(pc_topic_models, function(model) {
  doc_topic_distr <- as.data.frame(posterior(model)$topics)
  doc_topic_distr$equity_submission <- pc_equity_submissions[rownames(doc_topic_distr)]
  doc_topic_distr
})
...
plot_heatmap <- function(model, model_index) {
  # Get the document-topic distributions from the model
  doc_topic_distributions <- posterior(model)$topics

  # Convert to data frame and tidy up for ggplot
  doc_topic_distributions <- as.data.frame(doc_topic_distributions)
  doc_topic_distributions$document <- rownames(doc_topic_distributions)
  doc_topic_distributions_long <- reshape2::melt(doc_topic_distributions, id.vars = "document", 
                                                 variable.name = "topic", value.name = "proportion")

  # Create the plot
  p <- ggplot(doc_topic_distributions_long, aes(x = topic, y = document, fill = proportion)) +
    geom_tile(show.legend = FALSE) +
    scale_fill_gradient(low = "white", high = "steelblue") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.text.y = element_blank()) +
    labs(x = "Topic", y = "", fill = "Proportion", 
         title = paste0("Document-Topic Distributions for Model ", model_index))
...
pc_plots <- map2(pc_topic_models, 1:length(pc_topic_models), plot_heatmap)

# Now `plots` is a list of Plotly objects, each one is a heatmap for a different model.
# You can view a plot by indexing into the list, like so:
print(pc_plots[[2]])  # Print the first plot
```

```{r}
# HEATMAP WITH TOOLTIP FOR DOCUMENT NAME AND EQUITY MENTION STATUS

# Define function to create heatmap
plot_heatmap <- function(model, model_index) {
  # Get the document-topic distributions from the model
  doc_topic_distributions <- as.data.frame(posterior(model)$topics)
  doc_topic_distributions$document <- rownames(doc_topic_distributions)

  # Add the equity_submission status
  doc_topic_distributions$equity_submission <- pc_equity_submissions[doc_topic_distributions$document, ]

  # Convert to data frame and tidy up for ggplot
  doc_topic_distributions_long <- reshape2::melt(doc_topic_distributions, id.vars = c("document", "equity_submission"), 
                                                 variable.name = "topic", value.name = "proportion")

  # Create the plot
  p <- ggplot(doc_topic_distributions_long, aes(x = topic, y = document, fill = proportion, 
                                                text = paste('Document:', document, '<br>',
                                                             'Equity Submission:', equity_submission, '<br>',
...
```{r}
# REORDERING IN HEAT MAP - NOT YET WORKING

plot_heatmap <- function(model, model_index) {
  # Get the document-topic distributions from the model
  doc_topic_distributions <- as.data.frame(posterior(model)$topics)
  
  # Add equity submission status
  doc_topic_distributions$equity_submission <- as.logical(pc_equity_submissions[rownames(doc_topic_distributions)])
  ...
  # Create the plot
  p <- ggplot(doc_topic_distributions_long, aes(x = topic, y = document, fill = proportion)) +
    geom_tile(aes(fill = ifelse(equity_submission, proportion, NA)), show.legend = FALSE) +
    geom_tile(aes(fill = ifelse(!equity_submission, proportion, NA)), show.legend = FALSE) +
    scale_fill_gradientn(colors = c("white", "red"), na.value = "white", name = "Equity Submissions") +
    scale_fill_gradientn(colors = c("white", "steelblue"), na.value = "white", name = "Non-Equity Submissions") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1), axis.text.y = element_blank()) +
    labs(x = "Topic", y = "", title = paste0("Document-Topic Distributions for Model ", model_index))
  
  # Convert to Plotly for interactivity
  p <- ggplotly(p, tooltip = c("y", "fill", "equity_submission"))
...
# Sefton review submissions
## Bring in text from PDFs

```{r}

sefton_directory <- "docs_sefton"

sefton_submissions_text <- map_df(sefton_directory, read_pdfs_in_dir)
...
# create subdirectory
sefton_concord_subdirectory <- "output_sefton/sefton_concordance"

dir.create(sefton_concord_subdirectory, recursive = T, showWarnings = F)

# Define key word patterns - done as pc_efj

# Tokenise the submissions and create a corpus
sefton_sub_tokens <- corpus(sefton_submissions_text$text, docnames = sefton_submissions_text$filename) %>%
  tokens(remove_punct = TRUE, remove_numbers = TRUE, remove_symbols = TRUE)

# Iterate over the keyword patterns and export as excel files, and collect data into one dataframe
sefton_all_data <- map_dfr(pc_efj, ~{
  # Generate KWIC concordance for the current pattern
  kwic_concordance <- sefton_sub_tokens %>%
    kwic(pattern = .x, valuetype = "regex", window = 30, case_insensitive = TRUE) %>%
    as.data.frame()

  # Create a filename for the Excel file
  file_name <- paste0(sefton_concord_subdirectory, "/", "sefton_concordance_", gsub("\\\\w\\*", "", .x), ".xlsx")
...
# Count the number of documents that contain each string pattern
sefton_doc_counts <- sefton_all_data %>%
  group_by(token) %>%
  summarise(num_docs = n_distinct(filename), 
            proportion = num_docs / n_distinct(sefton_submissions_text$filename))

# Count the number of times each string pattern appears in each document
sefton_token_counts <- sefton_all_data %>%
  count(filename, token)
...
```{r}
# additional packages working from https://ladal.edu.au/webcrawling.html
...
```{r}
mdb_guardian_url <- "https://www.theguardian.com/australia-news/murray-darling-basin"
# go to URL
pjs_session$go(mdb_guardian_url)
# render page
guardian_mdb_rendered_source <- pjs_session$getSource()
# download text and parse the source code into an XML object
guardian_mdb_html_document <- read_html(guardian_mdb_rendered_source)

page_numbers <- 1:18
mdb_guardian_base_url <- "https://www.theguardian.com/australia-news/murray-darling-basin?page="
mdb_guardian_paging_urls <- paste0(mdb_guardian_base_url, page_numbers)



guardian_mdb_links <- mdb_guardian_paging_urls %>%
  map(function(url) {
  ...
  unlist()  # to flatten the list into a vector
...
```{r}
# Webscraper from https://ladal.edu.au/webcrawling.html
...
```{r}
guardian_mdb_articles_2015_2023 <- guardian_mdb_links %>%
  map_dfr(function(link) {
    # scrape website
    article <- scrape_guardian_article(link)
    # return current article data.frame
    return(article)
  })
...
```{r}
# Pulling out concordances from the Guardian

guardian_subdirectory <- "guardian_articles/guardian_concordance"

dir.create(guardian_subdirectory, recursive = T, showWarnings = F)

# Tokenise the mdba texts and create a corpus
guardian_tokens <- quanteda::corpus(guardian_mdb_articles_2015_2023,
  docid_field = "url",
  text_field = "body",
  meta = list(),
...
map(pc_efj, ~{
  # Generate KWIC concordance for the current pattern
  kwic_concordance <- guardian_tokens %>%
    kwic(pattern = .x, valuetype = "regex", window = 30, case_insensitive = TRUE) %>%
    as.data.frame()

  # Create a filename for the Excel file
  file_name <- paste0(guardian_subdirectory, "/", "guardian_concordance_", gsub("\\\\w\\*", "", .x), ".xlsx")
...
guardian_dtm <- convert(guardian_dfm, to = "topicmodels")

```

```{r}
guardian_lda_model <- LDA(guardian_dtm, k =15, method = "Gibbs", control = list(seed = 1234))

#view terms

N <- 20
terms(guardian_lda_model, N)



```

# Comparing PC submissions and Guardian articles for similarity using UMAP

```{r}
guardian_umap <- umap(guardian_dfm)
pc_umap <- umap(pc_sub_dfm)
...
guardian_umap_data <- data.frame(UMAP1 = guardian_umap$layout[,1], UMAP2 = guardian_umap$layout[,2], Corpus = "Guardian")
pc_umap <- data.frame(UMAP1 = pc_umap$layout[,1], UMAP2 = pc_umap$layout[,2], Corpus = "PC")
pc_guardian_compare_umap <- rbind(guardian_umap_data, pc_umap)

# Plot
ggplot(pc_guardian_compare_umap, aes(x = UMAP1, y = UMAP2, color = Corpus)) + 
  geom_point(alpha = 0.6, size = 3) + 
  theme_minimal() +
  ggtitle("UMAP Guardian and Productivity Commisison corpora")
```

```{r}
# Calculate type-token ratio for each DFM
guardian_ttr <- ntype(guardian_dfm) / ntoken(guardian_dfm)
pc_sub_ttr <- ntype(pc_sub_dfm) / ntoken(pc_sub_dfm)

print(paste("Type-Token Ratio for Corpus 1: ", guardian_ttr))
print(paste("Type-Token Ratio for Corpus 2: ", pc_sub_ttr))

# Note that the type-token ratio will be closer to 1 for shorter texts - probably not meaningful for a comparison between newspaper articles and submissions
...
```{r}
mdb_abc_url <- "https://discover.abc.net.au/index.html#/?query=murray%20darling%20basin"
# go to URL
pjs_session$go(mdb_abc_url)
# render page
abc_mdb_rendered_source <- pjs_session$getSource()
# download text and parse the source code into an XML object
abc_mdb_html_document <- read_html(abc_mdb_rendered_source)

page_numbers <- 1:639
mdb_abc_base_url <- "https://discover.abc.net.au/index.html#/?query=murray%20darling%20basin&page="
mdb_abc_paging_urls <- paste0(mdb_abc_base_url, page_numbers)
```

```{r}

# This doesn't work - wrong structure for ABC page, which seems to be using more javascript to load and display content


abc_mdb_links <- mdb_abc_paging_urls %>%
  map(function(url) {
    # download and parse single overview page
    pjs_session$go(url)
    rendered_source <- pjs_session$getSource()
...
# inspect 
abc_mdb_links #provides an empty list
