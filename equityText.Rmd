...
words_of_interest_counts_mdba <- token_freqs_mdba[names(token_freqs) %in% words_of_interest_mdba]
...
# Looking at all toekns in the fcm

all_feature_counts_df <- dfm_mdba_nb_sub %>%
  as.matrix() %>%
  as.data.frame() %>%
  rownames_to_column(var = "original_token") %>%
  gather(key = "document", value = "frequency", -original_token) %>%
  group_by(original_token) %>%
  summarise(frequency = sum(frequency), .groups = "drop") %>%
  rename(token = original_token) %>%
  arrange(desc(frequency))


# calculate token frequencies, sort, and convert to data frame

all_feature_counts_df <- dfm(mdba_nb_sub_token) %>%
  colSums() %>%
