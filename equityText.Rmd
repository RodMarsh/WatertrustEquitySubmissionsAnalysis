...
# Assuming 'corpus' is your quanteda Corpus object
# Extract the texts from the corpus, lemmatize them, and create a new corpus
pc_sub_tokens_lemma <- texts(pc_sub_tokens) %>%
  sapply(lemmatize_spacy) %>%
  corpus()

# Perform lower-casing, stopword removal, and stemming
pc_sub_tokens_lemma <- pc_sub_tokens_lemma %>%
  tokens_tolower() %>%
  tokens_remove(stopwords("english")) %>%
  tokens_wordstem()

# Create DFM
dfm <- dfm(pc_sub_tokens_lemma)
...
pc_lda_model_terms_output_table <- pc_lda_model_20_top_terms_df %>%
  kable("html", caption = "Top Terms per Topic") %>%
  kable_styling(bootstrap_options = c("striped", "condensed"))

pc_lda_model_terms_output_table

# See the table_report.R script and rmd docs for exporting this kableExtra table.
```
